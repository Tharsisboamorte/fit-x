...
import com.project.fitx.utils.isPassword
        ...
        Column(
            ...
            DefaultButton(
                modifier = Modifier
                    ...
                    .height(50.dp),
                label = "Sign in",
                isEnabled = passwordTextValue.length >= 6 && isEmail(textFieldValue),
                onAction = {
                    if (!isEmail(textFieldValue) && viewModel.currentUser!!.isEmailVerified) {
                        isErrorEmail = true
                    } else {
                        if (!isPassword(passwordTextValue)) {
